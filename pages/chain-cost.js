...
async function loadFilterOptions() {
  try {
    const opts = await getChainFilterOptions();
...
async function loadRecords(filters = {}) {
  ...
  try {
    allRecords = await getChainCosts(filters);
...
function renderTable(records) {
  const container = document.getElementById('table-container');
  if (!records.length) {
    container.innerHTML = emptyState('', '尚無鍊條線材進貨紀錄，點右上角「新增進貨」開始記錄');
    return;
  }
      ...
      <table style="min-width:1100px">
        <thead>
          <tr>
            ...
            <th style="min-width:90px">1cm成本$</th>
...
async function deleteSelected() {
  ...
  try {
    await Promise.all(ids.map(id => deleteChainCost(id)));
...
function openAddModal() {
  ...
  resetAddForm();
  document.querySelector('#addModal .modal-title').textContent = '新增鍊條線材進貨';
...
function openEditRecord(id) {
  ...
  document.getElementById('a-costPerCm').value = record.costPerCm || '';
...
async function submitAdd() {
  ...
  const data = {
    ...
    costPerCm: parseFloat(get('a-costPerCm')) || 0,
    note: get('a-note')
  };
  ...
  try {
    ...
    if (editingRecordId) {
      await updateChainCost(editingRecordId, data);
      showToast('鍊條線材進貨紀錄已更新', 'success');
    } else {
      await addChainCost(data);
      showToast('鍊條線材進貨紀錄儲存成功！', 'success');
    }
...
function resetAddForm() {
  ['a-vendor','a-itemCode','a-productName','a-shopLink','a-imageUrl',
   'a-color','a-spec','a-costPerLot','a-pricePerPieceYuan','a-costPerCm','a-note'].forEach(id => {
...
async function deleteRecord(id) {
  if (!confirmDialog('確定要刪除這筆鍊條線材進貨紀錄嗎？')) return;
  try {
    await deleteChainCost(id);
...
async function submitImport() {
  ...
  if (failed === 0) {
    showToast(`成功匯入 ${success} 筆鍊條線材進貨紀錄`, 'success');
    closeUploadModal();
    await loadFilterOptions();
    await loadRecords();
  } else {
...
function downloadChainTemplate() {
  const data = [
    ['廠家','賣場Link','貨號','圖片連結','賣場商品名稱','顏色','規格','單品進貨成本$','單顆進價¥','1cm成本$','備註'],
  ];
  const ws = XLSX.utils.aoa_to_sheet(data);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, '鍊條線材進貨');
  XLSX.writeFile(wb, '鍊條線材進貨範本.xlsx');
}
